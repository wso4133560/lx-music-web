...
import {
  ...
  openRuntimeDevTools,
  quitRuntimeApp,
  setRuntimeFullScreen,
  subscribeRuntimeFocus,
  subscribeRuntimeSettingChanges,
...
const handle_key_down = ({ event, type, key }: LX.KeyDownEevent) => {
  ...
  if ((event.target as HTMLElement).tagName != 'INPUT') {
    if (isFullscreen.value) {
      ...
      void setRuntimeFullScreen(false).then(fullscreen => {
...
const handle_fullscreen = (event: LX.KeyDownEevent) => {
  ...
  void setRuntimeFullScreen(fullscreen).then(fullscreen => {
    isFullscreen.value = fullscreen
  })
}
...
export default () => {
  ...
  const rSetConfig = subscribeRuntimeSettingChanges(({ params: setting }) => {
    // console.log(config)
    mergeSetting(setting)
    window.app_event.configUpdate(setting)
  })

  const rFocus = subscribeRuntimeFocus(() => {
    clearDownKeys()
  })

  const rThemeChange = subscribeRuntimeThemeChanges(({ params: setting }) => {
    // console.log(setting)
  ...
  if (!isWebRuntime) {
    window.key_event.on(HOTKEY_COMMON.min.action, minRuntimeWindow)
    window.key_event.on(HOTKEY_COMMON.hide_toggle.action, toggleRuntimeWindowVisibility)
    window.key_event.on(HOTKEY_COMMON.close.action, quitRuntimeApp)
  ...
  onBeforeUnmount(() => {
    if (!isWebRuntime) {
      window.key_event.off(HOTKEY_COMMON.min.action, minRuntimeWindow)
      window.key_event.off(HOTKEY_COMMON.hide_toggle.action, toggleRuntimeWindowVisibility)
      window.key_event.off(HOTKEY_COMMON.close.action, quitRuntimeApp)
